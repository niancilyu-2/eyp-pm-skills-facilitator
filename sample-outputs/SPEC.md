...
## Design
- **Approach:** activate the core's dormant segment seam (real provider replacing the no-op
  service), add a **per-request segment assignment engine** (member/session → segment) and harden
...
- [Verified] Affected-subsystem sizes (estimate grounding).
  `git ls-files -- 'src/Umbraco.Core/**' | grep -E '\.cs$' | xargs wc -l | tail -1` → `48690 total`
  `git ls-files -- 'src/Umbraco.Web.UI.Client/**' | grep -Ev '/(node_modules|dist|wwwroot|bin|obj)/' | grep -E '\.(ts|tsx|js|html)$' | grep -v '\.min\.' | xargs wc -l | tail -1` → `39584 total`
...
- [Verified absent] Segment handling in the publish pipeline — lifecycle enforcement is new work,
  not configuration. `grep -ci "segment" src/Umbraco.Core/Services/ContentPublishingService.cs` → `0`
- [Verified absent] Server-side segment assignment on request paths — the inbound segment context
  ...
  — and none of the 5 `ISegmentService` files (list above) sit on the render path. Implication
  (design body): the studio needs an assignment engine, and unhardened headers are a stopped-variant
  bypass.

### Deprecation & staleness check (mandatory)
- [Verified] The seam is deprecated-in-motion.
